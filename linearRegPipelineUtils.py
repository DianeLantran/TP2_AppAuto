...
def pipeline(dataset, column):
    ...
    X_train, X_test, y_train, y_test = splitTrainTest(cols, column, test_size=0.2, random_state=42)
...
class multipleLinReg:
    ...
    def fit(self, data, column):
        ...
        # calcule les
        coefficients = np.linalg.inv(X_with_intercept.T @ X_with_intercept) @ X_with_intercept.T @ column
    ...
    def predict(self, data):
        ...
        # Compute predictions
        predictions = data_with_intercept @ np.concatenate([[self.intercept_], self.coef_])
